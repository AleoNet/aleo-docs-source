...
```leo title="No Type Parameters - Void Return"
_dynamic_call(prog, net, func, x);
```

Inputs are `private` by default:

```leo title="Return Type Only"
...
```leo title="Dynamic Record Inputs and Outputs"
