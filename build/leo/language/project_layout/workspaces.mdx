...
Alternatively, members can use [local path dependencies](../../guides/dependencies.mdx#local-dependencies) with `"location": "local"` and an explicit relative `path`. Workspace dependencies are preferred because they are shorter to write and do not break if you reorganize directories.
