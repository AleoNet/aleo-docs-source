...
This guide explains how to use the **Delegated Proving Service (DPS)** to generate proofs for Aleo programs via Provable's infrastructure. For discovering and querying records that belong to your account, see the [Record Scanning](./record-scanning.mdx) guide.

:::info Network
Examples on this page target Mainnet (`@provablehq/sdk/mainnet.js`). For Testnet, swap the import for `@provablehq/sdk/testnet.js` — see [Network Selection](../getting-started.mdx#network-selection) for details.
:::
...
1. Client requests an ephemeral X25519 public key from the prover: [`GET {proverBase}/pubkey`](https://docs.provable.com/docs/api/services/get-prove-pubkey).
2. Client builds a *proving request* (authorization, optional fee authorization, broadcast flag), encrypts it via a libsodium cryptobox (e.g. with [encryptProvingRequest](https://github.com/ProvableHQ/sdk/blob/424e8e9b91a6cbf7d096b9327ebdec2b9513cbe8/sdk/src/security.ts#L25)), and sends it to: [`POST {proverBase}/prove/encrypted`](https://docs.provable.com/docs/api/services/post-prove-encrypted) with `{ "key_id": string, "ciphertext": string }`.
...
|------|--------|----------|--------------|----------|
| [`/pubkey`](https://docs.provable.com/docs/api/services/get-prove-pubkey) | GET | Ephemeral public key for encrypted proving | — | `{ "key_id": string, "public_key": string }` |
| [`/prove/encrypted`](https://docs.provable.com/docs/api/services/post-prove-encrypted) | POST | Submit proving request (encrypted) | `{ "key_id": string, "ciphertext": string }` | Same as `/prove` on success |
...
5. **Record scanner with ProgramManager:** You can pass a [Record Scanner](./record-scanning.mdx) (or any `RecordProvider`) into `ProgramManager`. The manager will then use it to resolve fee records (and inputs) when building executions or proving requests, so you do not have to fetch fee records manually.
