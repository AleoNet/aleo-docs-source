...
| **State Model**      | Public key-value `mapping` and private `record` [storage](/learn/core-concepts/public-and-private-state) | Key-value contract `storage`, temporary `transient` storage, temporary `memory` and read-only `calldata` |
...
| **Token Standard**   | [ARC-21](https://vote.aleo.org/p/21) (existing), [ARC-20](https://github.com/ProvableHQ/ARCs/discussions/124) / [ARC-22](https://github.com/ProvableHQ/ARCs/discussions/125) (draft) | ERC-20 |
| **NFT Standard**     | [ARC-721](https://vote.aleo.org/p/721) | ERC721 |
| **Upgradability**    | Native upgradability via [ARC-0006](https://vote.aleo.org/p/arc-0006) (`constructor` + `edition` + `checksum`) | Via proxy patterns (Transparent, UUPS, Beacon) |
