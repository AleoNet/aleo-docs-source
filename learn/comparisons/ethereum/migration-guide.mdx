...
sidebar_position: 2
description: A guide for migrating from Ethereum to Aleo.
---
...
:::info Leo version
Code samples on this page use Leo 4.0 syntax (`fn`, `final`, `Final`). If you're working with Leo 3.5 or earlier, see the [Migration Guide (3.5 → 4.0)](/build/leo/documentation/guides/migration_3_5_to_4_0).
:::
...
program storage.aleo {
    ...
    // Function parameters and returns are private by default unless marked public
    fn process_data(public amount: u64) -> (Token, Final) {
        ...
        return (token, final {
            let current_balance: u64 = balances.get_or_use(self.caller, 0u64);
            balances.set(self.caller, current_balance + amount_loc);
        });
    }
}
```
...
- **Leo Privacy**: `public` vs `private` determines actual cryptographic privacy - whether data is stored on-chain or off-chain in [records](/learn/core-concepts/public-and-private-state)
...
program delete_example.aleo {
    ...
    fn remove_balance(user: address) -> Final {
        return final {
            ...
            let exists: bool = balances.contains(user);
            if exists {
...
- **Leo**: Only supports removing entries from mappings using the `.remove()` method
...
- **Key Existence Check**: Leo provides the `.contains()` method to check if a key exists in a mapping before operations
...
program example.aleo {
    // Entry point: externally callable, off-chain execution
...
// Helper function, declared outside the program {} block
// Can only compute - no state access, no external calls
fn internal_helper(input: u32) -> u32 {
    let doubled: u32 = multiplier(input, 2u32);
    return doubled;
}
...
**Key Differences:**
- **Externally Callable Functions**: In Leo, only `fn` declarations inside the `program {}` block are externally callable entry points. Helper `fn` declarations outside `program {}` are internal and can only be called from within the program
- **Leo's Computation-Only Functions**: Helper functions in Leo are similar to Solidity's `pure` functions - they can only perform computations and cannot access state variables or make external calls
...
- **Unified Keyword**: Leo 3.5 distinguished entry points (`transition`), helpers (`function`), and inlined helpers (`inline`). Leo 4.0 unifies all of these under `fn`; see the [Migration Guide (3.5 → 4.0)](/build/leo/documentation/guides/migration_3_5_to_4_0) for details.
...
**Leo's Approach:**
Leo doesn't have modifiers, but similar functionality can be achieved using helper functions declared outside the `program {}` block:
...
Leo uses `final` blocks and functions for on-chain execution:

```leo
program defi_example.aleo {
    mapping user_balances: address => u64;
    
    // Entry point: can return a Final value that runs on-chain
    fn deposit_and_update(public amount: u64) -> Final {
        ...
        // Return a final block for on-chain execution
        return final {
...
**Key Concepts:**
- **Two Execution Contexts**: Leo functions run in either a proof context (private, off-chain, generates a ZK proof) or a finalization context (public, on-chain, modifies state). Entry points that need to update on-chain state return `Final` and include a `final { ... }` block
- **`Final` Values**: A `final { ... }` block is lifted into a standalone on-chain finalization function that executes after the proof is verified, allowing access to mapping values that the proof context cannot access. See the [Migration Guide (3.5 → 4.0)](/build/leo/documentation/guides/migration_3_5_to_4_0#asyncfinalize-to-final) for the full execution model

### Call Restrictions

Leo enforces strict rules about function call hierarchy:

**Call Flow:** entry point `fn` (inside `program {}`) → helper `fn` (outside `program {}`) → other helper `fn` | entry point `fn` → external entry point `fn`

**Call Flow Rules:**
- An entry point `fn` can call a helper `fn` or an external program's entry point `fn`.
- A helper `fn` can call another helper `fn`.
...
program call_hierarchy.aleo {
    // Entry point can call: helper functions, external program entry points
    fn main_entry(input: u32) -> u32 {
        let result1: u32 = helper_function(input);      // Valid
        let result2: u32 = doubler_helper(input);       // Valid
        let result3: u32 = external_program.aleo::some_fn(input); // Valid
        return result1 + result2 + result3;
    }
}

// Helper functions can call other helper functions
fn helper_function(input: u32) -> u32 {
    let processed: u32 = doubler_helper(input);      // Valid
    // let invalid: u32 = main_entry(input);          // Invalid - cannot call an entry point
    return processed;
}

fn doubler_helper(input: u32) -> u32 {
    return input * 2u32;
}
```
...
fn secure_random() -> Final {
    return final {
        // Supports ChaCha random - only available in the finalization context
...
program global_access.aleo {
    fn get_block_info() -> Final {
        return final {
            // Available within the finalization context only
...
**Key Differences:**
- **Limited Scope**: Leo's block properties are only available within `final` blocks (the finalization context)
...
program static_calls.aleo {
    fn transfer_credits(
        ...
        return (tuple.0, final {
            tuple.1.run();
        });
    }
    
    // Can query public state from other programs within the finalization context
    fn get_external_balance(user: address) -> Final {
        return final {
...
- **Standalone Programs**: Aleo Instructions can be written as standalone programs in `.aleo` files, enabling fine-grained control over program execution and circuit design at a low level. For more details about Aleo Instructions, see the [Aleo Instructions Overview](/build/aleo-instructions/overview).
