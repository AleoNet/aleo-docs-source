...
The Leo test framework executes tests via the real VM, so onchain state (mappings, storage) is fully supported in `@test fn` functions — no special syntax is required. Call entry functions that return `Final` the same way as any other function; the finalization will be executed as part of the test run.
...
See the `leo test` CLI documentation [here](../cli/test.mdx)

## Running a Devnet

A local devnet can be a heavyweight but reliable way to test your application on Aleo.

For more information, refer to the [Devnet](./devnet.mdx) guide
